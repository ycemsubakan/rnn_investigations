    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):  # "GRUCell"
            ...
            if self.wform == 'full' or self.wform == 'diag_to_full':
                ...
                i,j,f,o = tf.split(res_wbiases,num_or_size_splits=4,axis=1) 
            elif self.wform == 'fully_diagonal':
                h_concat = tf.concat([h,h,h,h],axis=1)
                x_concat = tf.concat([inputs,inputs,inputs,inputs],axis=1)
                ht_tensor = tf.stack([h_concat, x_concat])

                #W_res = tf.multiply(h_concat, mats[0])
                #U_res = tf.multiply(x_concat, mats[1])
                res = tf.einsum('ibk,ik->bk',ht_tensor,mats)
                #res = tf.add(W_res,U_res)
                res_wbiases = tf.nn.bias_add(res, biases)
...
class LDSCell(tf.contrib.rnn.RNNCell):    
    """LDS Cell """
    ...
    def __call__(self, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):  # "LDSCell"
                        
            init = self.init
           
            W = tf.get_variable("W", shape = [self._num_units, self._num_units],
                                initializer = self.init) 
            b = tf.get_variable("b", shape = [1, self._num_units],
                                initializer = self.init) 
                        
            new_h = tf.nn.tanh( tf.matmul(state,W) + b ) 
        return new_h, new_h
...
class rnn(object):
    ...
    def define_model(self, x, seqlens ,dropout_kps = tf.constant([1,1])):  
        ...
        if model in LDS:
            ...
            # I need to write the fully diagonal case here 
            if wform == 'fully_diagonal':
                ...
                x_einprod = tf.einsum('kl,tbl->tbk',U,x)
                x = tf.nn.tanh( x_einprod + b)
