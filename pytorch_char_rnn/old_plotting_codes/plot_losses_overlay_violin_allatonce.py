...
def makeviolinplot(*arg):
    ...
    for i, info in enumerate(zip(model_infos,picks)):
        ...
        plt.plot( ss_ids, mean_y, '-' + colors[i] , label =  info[0][0]['wform'][0].upper() + ',#p=' + '{:2.1f}e5'.format(n_params/(1e5))) 
        plt.ylim(yrange)
        plt.xlim( [0,T] ) 
...
models = ['mod_rnn', 'mod_lstm', 'gated_wf']
datasets = ['JSBChorales','Piano-midi','Nottingham','MuseData']
load_datasets = ['JSB Chorales','Piano-midi.de','Nottingham','MuseData']
optimizers = ['Adam','RMSprop']

for optimizer, model, sets in it.product(optimizers,models,zip(datasets,load_datasets)):
    dataset, load_dataset = sets[0], sets[1]

    directory = 'temp_data2/' if optimizer == 'Adam' else 'temp_data/' 

    print('Printing: ',model,dataset)
    ...
    N = np.min([len(model1),len(model2)]) #number of configurations to be used in the plot
    ...
    if optimizer == 'Adam':
        fig.savefig('../WASPAA2017/paper/'+ model + dataset  +  '.eps', dpi=100)
